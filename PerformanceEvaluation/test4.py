...
x = np.linspace(20,150,2000)
#从1到10随机产生200个值
y = 1-(1-3*5*10**(-3))**(x)
x4 = np.linspace(20,150,14)
y5 = 1-(1-3*5*10**(-3))**(x4)
# y2 = 1 - (1-3*5*10**(-3))**(x-1)*(x-(x-1)*(1-3*5*10**(-3)))
x3 = []
for i in range(2,16):
    x3.append(i*10)
x3 = np.array(x3)
y3 = [0.004115417,0.010265417
,0.016916786
,0.027205214
,0.035228307
,0.047203148
...
y_major_locator = MultipleLocator(1*10**(-1))
...
# l2, = plt.plot(x,y2)
l3, = plt.plot(x3,y4)
plt.legend(handles = [l1,l3] ,labels = ['Uncoded','DNA-LC'],loc = 'best')
plt.scatter(x4,y5,c = 'r',s = 10)
plt.scatter(x3,y3,c = 'r',s = 10)
plt.title('varied by length')

plt.savefig('plot123_4.png', dpi=1000)
# 'best'：自动选择最佳位置。
