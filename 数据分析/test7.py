...
#从1到10随机产生200个值
y2 = 1 - (1-3*x*10**(-3))**(24)*(25-24*(1-3*x*10**(-3)))
x4 = np.linspace(1,10,10)
y5 = 1 - (1-3*x4*10**(-3))**(24)*(25-24*(1-3*x4*10**(-3)))
x3 = []
for i in range(10):
    x3.append((i+1))
x3 = np.array(x3)
y3 = [0.000841667,0.003522917,0.0070675,0.01188625,0.018262358,0.025436125,0.031714472,0.041183793,0.051678296,0.061133577]
def f_fit(x,y_fit):
    a,b,c=y_fit.tolist()
    return a*x**2+b*x+c
...
plt.title('message = 33 bits')

plt.savefig('plot123_1.png', dpi=1000)
# 'best'：自动选择最佳位置。
